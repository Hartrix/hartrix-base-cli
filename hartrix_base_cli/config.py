...
configPath=pathJoin(path,"config")
...
async def setupUser():
    ...
    if not os.path.exists(pathJoin(configPath,"users.json")): 
        ...
        with aiofiles.open(pathJoin(configPath,"users.json"),"w",encoding="utf-8") as f:
            await f.write(json.dumps([],indent=2,ensure_ascii=False))
            await f.close()
    
    logger.info("(users.json) 读取用户信息...")
    with aiofiles.open(pathJoin(path,"users.json"),"r",encoding="utf-8") as f:
...
async def setup():
    ...
    if not os.path.exists(configPath):
        os.makedirs(configPath,666,True)
